...
import { Event } from '../../../@core/data/events/event.model';
...
export class DDECalendarComponent implements OnInit {
  ...
  ngOnInit() {
    this.events$ = this.eventsService.getEventsByFilter('').pipe(
      map(({ data }) => {
        return data['eventos'].map((event: Event) => {
          return {
            title: event.titulo,
